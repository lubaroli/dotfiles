...
__conda_setup="$(CONDA_REPORT_ERRORS=false '/Users/lubaroli/opt/miniconda3/bin/conda' shell.bash hook 2> /dev/null)"
if [ $? -eq 0 ]; then
    \eval "$__conda_setup"
else
    if [ -f "/Users/lubaroli/opt/miniconda3/etc/profile.d/conda.sh" ]; then
# . "/Users/lubaroli/opt/miniconda3/etc/profile.d/conda.sh"  # commented out by conda initialize
        CONDA_CHANGEPS1=false conda activate base
    else
        \export PATH="/Users/lubaroli/opt/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda init <<<
